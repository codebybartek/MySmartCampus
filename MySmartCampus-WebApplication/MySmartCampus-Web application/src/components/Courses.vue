<template>
   <div class="container navbar-offset-top">
      ...
      <Course class="single_hidden" v-if="showSingleContent" v-bind:class="{show_single_content: showSingleContent}" :course_props="course_props" v-on:closeSingleContent="closeSingleContent()" @setAlert="setAlert"/>

      <Frequence class="single_hidden" v-if="showSingleFrequenceContent" v-bind:class="{show_single_content: showSingleFrequenceContent}" :course_props="course_props" v-on:closeSingleContent="closeSingleContent()"/>
      ...
      <EditCourse class="single_hidden" v-if="showEditFormContent" v-bind:class="{show_edit_form_content: showEditFormContent}" :course_props="course_props" v-on:closeSingleContent="closeSingleContent()" @setAlert="setAlert"/>
          ...
          <slide v-for="(course, index) in courses" :index="index" :style="{ height: article_height}" v-bind:class="{ current: index==courses.length-1 && !was_changed}" :key="index">
            <article  class="course" ref="articles_height" :style="{ height: article_height}">
              ...
              <ul class="course_activities">
                  <h4>Activities:</h4>
                  <li v-for="(activity, index) in course.activities" v-if="index < 5 ">
              ...
              <a v-if="is_professor" class="button_show_frequence" v-on:click="showSingleFrequence(course)" >Frequence <i class="fa fa-angle-double-right" aria-hidden="true"></i></a>
              ...
              <a v-if="is_professor" class="button_edit" v-on:click="editFormContent(course)"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</a>
            </article>
          </slide>
        </carousel-3d>
        </section>
...
import Frequence from './Frequence.vue';
import AddCourse from './add_forms/AddCourse.vue';
import EditCourse from './edit_forms/EditCourse.vue';
...
export default {
  ...
  data () {
    return {
      ...
      showSingleFrequenceContent: false,
  ...
  methods: {
      ...
      showSingleFrequence(course_props){
        this.course_props = course_props;
        this.showSingleFrequenceContent = true;
      },
      closeSingleContent(){
        ...
        this.showSingleFrequenceContent = false;
      },
      editFormContent(course_props){
        this.course_props = course_props;
