    ...
    <div class="center_content">
      ...
      <table class="table table-hover">
        ...
        <tr v-if="course.activities.length > 0" v-for="(activity, index) in course.activities">
          ...
          <td><router-link :to="'./activity/' + activity.hash ">view <i class="fa fa-angle-double-right" aria-hidden="true"></i></router-link></td>
        </tr>
      </table>
      <h4>News:</h4>
      <table class="table table-hover">
        ...
        <tr v-if="course.news.length > 0" v-for="(news, index) in course.news">
          ...
          <td>{{news.content}}</td>
          <td><span class="delete" v-on:click=deleteNews(news.id)><i class="fa fa-trash-o" aria-hidden="true"></i> </span></td>
          <td><router-link :to="'./news/' + news.hash ">view <i class="fa fa-angle-double-right" aria-hidden="true"></i></router-link></td>
        </tr>
      </table>
    </div>
  </div>
        ...
        console.log(id);
         ...
         .then(function (response) {
            if(this.course_hash){
              ...
              for(let i = 0; i <this.course.activities.length; i++){
                  if(this.course.activities[i].id === id){
                      this.course.activities.splice(i, 1);
                  }
              }
            }
        ...
        console.log(id);
         ...
         .then(function (response) {
            if(this.course_hash){
              ...
              for(let i = 0; i <this.course.news.length; i++){
                  if(this.course.news[i].id === id){
                      this.course.news.splice(i, 1);
                  }
              }
            }
            let alert = {content: response.data['deleted'], alertClass: "danger"};
