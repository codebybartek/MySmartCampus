<template>
   <div class="container navbar-offset-top">
      ...
      <EditSubject class="single_hidden" v-if="showEditFormContent" v-bind:class="{show_edit_form_content: showEditFormContent}" :subject_props="subject_props" v-on:closeSingleContent="closeSingleContent()" @setAlert="setAlert"/>
        ...
        <carousel-3d @after-slide-change="onAfterSlideChange" :style="{ height: article_height}" :controls-visible="true" :controls-next-html="'&#10093;'" :controls-prev-html="'&#10092;'">
          <slide v-for="(subject, index) in subjects" :index="index" :style="{ height: article_height}" v-bind:class="{ current: index==subjects.length-1 && !was_changed}" :key="index">
            <article  class="subject" ref="articles_height" :style="{ height: article_height}">
              <h2>{{subject.name}}</h2>
              <ul class="subject_courses" v-if="is_professor">
                  <h4>Courses:</h4>
              ...
              <a v-if="is_professor" class="button_edit" v-on:click="editFormContent(subject)"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</a>
            </article>
          </slide>
        </carousel-3d>
        </section>
...
import EditSubject from './edit_forms/EditSubject.vue';
