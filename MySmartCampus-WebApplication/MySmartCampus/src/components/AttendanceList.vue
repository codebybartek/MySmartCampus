...
export default {
  ...
  methods: {
    ...
    getStudentsAll(){
      console.log(this.activity.course.hash);
      axios.get(this.$store.getters.getUrl + '/groups/' + this.activity.course.group_id)
      .then(function (response) {
    ...
    addToList(id){
      if(this.isOnList(id)){
        ...
        this.attandanceList.push({user_id: id, activity_id: this.activity.id, data_presence: dateTime});
    ...
    addList(){
      ...
      /*axios.put(this.$store.getters.getUrl + 'api/activities/' + this.activity.hash, this.activity).then((response) =>{
